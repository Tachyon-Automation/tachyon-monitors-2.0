...
async function monitor(sku) {
    try {
        ...
        let req = `https://www.hibbett.com/on/demandware.store/Sites-Hibbett-US-Site/default/Stylitics-ShowProductDetails;.js?pid=${sku}&pid=${v4()}`//request url
        ...
        if (root.querySelector('.price-sales')) {
            ...
            try { image = 'https:' + root.querySelector('img').attributes.src.split(' ').join('').replace('thumb', 'medium') } catch (e) { }
            let color = root.querySelector('img').attributes.src.split(' ').join('').replace('?$thumb$', '').split('_')[1]

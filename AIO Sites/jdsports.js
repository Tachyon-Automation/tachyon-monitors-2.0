...
async function monitor(sku) {
    try {
        ...
        if (inStock) {
            let req = `https://www.jdsports.com/store/recommendations/json/productRecommendationsEndecaJson.jsp?renderType=pdp&products=${productID}:${styleID}:${colorID}%7C`//request url
            let set = await helper.requestJson(req, method, proxy, headers) //request function

...
const CHANNEL = '810926568943517706' //channel id
const site = 'FOOTLOCKERUS'; //site name
...
async function monitor(sku) {
    try {
        ...
        let req = `https://www-footlocker-com.translate.goog/zgw/product-core/v1/pdp/sku/${sku}?_x_tr_sl=en&_x_tr_tl=el&_x_tr_hl=en&_x_tr_pto=wapp`//request url
        ...
        if (body.inventory.inventoryAvailable == true) {
            let inStock = false;
            let url = `https://www.footlocker.com/product/Tachyon/${sku}.html#Tachyon`//product url
            ...
            for (let variant of variants) {
                ...
                if (!oldSizeList.includes(variant.productWebKey)) {
                    sizes += `[${variant.size}](https://www.footlocker.com/product/~/${sku}.html?size=${variant.size}) - ${variant.productWebKey}\n`
                    stock++
                    inStock = true;
                }
            }

...
const helper = {
    ...
    requestJson: async function (site, method, proxy, headers) {
        try {
            ...
            let response = await fetch(site, { method: method, headers: headers, signal: controller.signal }, await new HTTPSProxyAgent(proxy))
    ...
    postAIO: async function (url, title, sku, price, image, sizeright, sizeleft, stock, group, site, version) {
        ...
        try {
            let response = await fetch(group[site], { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }, await new HTTPSProxyAgent(proxy))
    ...
    discordbot: async function (CHANNEL, PRODUCTS) {
